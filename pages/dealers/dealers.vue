<template>
	<view class="dealers">
		<view class="dealers_header">
			<view class="header_address" @click="changeaddress">
				{{address}}
				<u-icon v-if="!isaddress" name="arrow-down-fill" color="#282d3b" size="24rpx"></u-icon>
				<u-icon v-if="isaddress" name="arrow-up-fill" color="#282d3b" size="24rpx"></u-icon>
			</view>
			<view class="header_input">
				<u--input
					prefixIcon="search"
				    placeholder="请输入特约店名称"
				    border="surround"
				    v-model="value"
				    @change="changevalue"
				  ></u--input>
		...
		<view class="dealers_list">
			<view class="list_item" v-for="(item,index) in listitemdata" :key="index">
				<view class="item_img" :key="index">
					<image :src="item.img"></image>
				</view>
				<view class="item_center">
					<view class="center_name">{{item.name}}</view>
					<view class="center_address">{{item.address}}</view>
					<view class="center_distance">距离您 <text>{{item.distance}}</text></view>
				</view>
				<view class="item_gohere">
					<view class="gohere_img">
						<image src="http://cq.ymukj.cn/dealers_gohere_img.png" alt="">
						<text>去这里</text>
					</view>
				</view>
			</view>
		</view>
		 <u-picker :show="isaddress" ref="uPicker" :columns="columns" @change="changeHandler" @confirm="confirmaddress" @cancel="canceladdress"></u-picker>
	</view>
</template>
		...
		data() {
			return {
				...
				listitemdata:[
					{
						id:1,
						img:'http://cq.ymukj.cn/dealers_item_img.png',
						name:'武汉安琪',
						address:'湖北省武汉市汉阳区龙阳大道1号',
						distance:'5.1km'
					},
					{
						id:2,
...
<style lang="scss">
/deep/.header_input	.u-input__content__field{
	font-size: 28rpx !important;
	font-family: PingFangSC-Regular, PingFang SC !important;
	font-weight: 400 !important;
	line-height: 40rpx !important;
}
/deep/.header_input .u-input--radius{
	width: 566rpx;
	background: #F1F2F3;
	border-radius: 44rpx;
	border: 0;
...
/deep/.header_input  .u-icon__icon{
	margin-left: 20rpx;
}
/deep/.header_address .u-icon__icon{
